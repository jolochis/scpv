...
function activarInteraccionUsuario(c){
	...
	if($(c).val()=="Iniciar"){	
		...
		*/
		snap = new OpenLayers.Control.Snapping({
                layer: capaAforos,
                targets: [interSeleccion,seleccion],
                greedy: true
		});
	...
	//										alert("Valido: "+simil.igual);
							...
							{
								//Aforos
								$.post("peticiones.php","action=getSubLinea&line="+clic.geometry+"&point1=.9&point2=.1",function(f)
								{
									var jsondecode = new OpenLayers.Format.GeoJSON();
								...
								//capaAforos.addFeatures(data.Feature);
							}
