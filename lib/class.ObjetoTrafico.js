...
ObjetoTrafico.prototype.getNthPosition = function(pointN){
	return this.posiciones[pointN];
};
...
ObjetoTrafico.prototype.anadirPosicion = function(lon,lat,linea){
	...
	if(this.tipo == "aforo"){
		selectedLayer = capaAforos;
		if(typeof this.posiciones[0] != "undefined"){

			this.posiciones.push(new OpenLayers.LonLat(lon,lat));
			...
			if (p == null){
				p = this.posiciones.push(new OpenLayers.LonLat(lon,lat));				
			}	
			else{
				linea2=this.linea;			 
			}
...
/*ObjetoTrafico.prototype.actualizarSemaforo = function(lon,lat){
	this.posiciones[0].lon = lon;
	this.posiciones[0].lat= lat;
	console.log("Añadiendo semaforo en "+lon+","+lat);
	if(typeof this.semaforoMarker != "undefined"){ 
		this.semaforoMarker = new OpenLayers.Geometry.Point(lon,lat);
		selectedFeature={feature:this.semaforoMarker,id:null};
		capaSemaforo.addFeatures(this.semaforoMarker);
	}
	//if(typeof this.semaforoMarker != "defined"){ semaforosLayer.addMarker(this.semaforoMarker) }
	
	else{
	    capaSemaforos.destroyFeatures(this.semaforoMarker);
	}
};*/
...
ObjetoTrafico.prototype.actualizarSemaforo = function(){
	...
	if(typeof this.semaforoMarker != "undefined"){
		if(lon && lat){
			this.posiciones[0].lon = lon;
			this.posiciones[0].lat= lat;
			this.semaforoMarker = new OpenLayers.Feature.Vector(
				 new OpenLayers.Geometry.Point( this.posiciones[0].lon,this.posiciones[0].lat));
		}
		else{
			this.semaforoMarker = new OpenLayers.Feature.Vector(
				new OpenLayers.Geometry.Point( this.posiciones[0].lon,this.posiciones[0].lat));
		}
		this.semaforoMarker.data["trafficData"] = {};
		capaSemaforos.addFeatures(this.semaforoMarker);
		selectedFeature={f:this.semaforoMarker};
	}
	else{
	    capaSemaforos.removeFeature([this.semaforoMarker]); 
	}
};

ObjetoTrafico.prototype.actualizarTope = function(lon,lat){
	console.log("Añadiendo tope en "+this.posiciones[0].lon+","+this.posiciones[0].lat);
	
	if(typeof this.topeMarker != "undefined"){
		if(lon && lat){
			this.posiciones[0].lon = lon;
			this.posiciones[0].lat= lat;
			this.topeMarker = new OpenLayers.Feature.Vector(
				 new OpenLayers.Geometry.Point( this.posiciones[0].lon,this.posiciones[0].lat));
		}
		else{
		this.topeMarker = new OpenLayers.Feature.Vector(
		...
		this.topeMarker.data["trafficData"] = {};
		capaTopes.addFeatures(this.topeMarker);		
		selectedFeature={f:this.topeMarker};
	}
	else{
	    capaTopes.removeFeature([this.topeMarker]); 
	}
};
...
ObjetoTrafico.prototype.actualizarpAuto = function(){
	...
	if(typeof this.pautoMarker != "undefined"){ 
		this.pautoMarker = new OpenLayers.Marker(this.posiciones[0],nuevopAutoIcon.clone());
...
ObjetoTrafico.prototype.actualizarAforo = function(){
	...
	if(this.posiciones.length >= 2){//this.posiciones[this.posiciones.length-1].length >= 2){
		//dibuja el segmento.
		if(typeof this.direccionesAforos[this.posiciones.length-1] != "undefined") 
			capaAforos.removeFeatures([this.direccionesAforos[this.posiciones.length-1]]);
...
ObjetoTrafico.prototype.actualizarDesnivel = function(){
	...
	if(this.posiciones.length >= 2){
		...
		this.lineaDesnivel = new OpenLayers.Feature.Vector({
			"geometry":new OpenLayers.Geometry.LineString(this.posicionesToPoint(this.posiciones)),
			"trafficObject":{},
			"style":this.desnivelStyle
...
ObjetoTrafico.prototype.limpiarCapas = function(){
	...
	capaSemaforos.markers = new Array();
...
ObjetoTrafico.prototype.getPosiciones = function(){
	...
	if(this.tipo == "aforo"){
		...
		lin = "LINESTRING("+linea.join(",")+")";
				
		return lin;		
	}
	else return "POINT("+this.posiciones[0].lon+" "+this.posiciones[0].lat+")";		
	
};


ObjetoTrafico.prototype.eliminaObjeto = function(id){
	var post = {action:"eliminarObjeto",
		o:id};
	$.post("antragen.php",post,function(data){
			
		},"json");
};


ObjetoTrafico.prototype.actualizaObjeto = function(ido,idc,type){
	switch(type){
		case "semaforo":
			this.configuracionInicial = new ConfiguracionObjeto($("#luzini").val(),$("#tinsem").val(),$("#tfinsem").val(),1);
				this.configuraciones = [
				new ConfiguracionObjeto("red",0,$("#lrd").val(),0),
				new ConfiguracionObjeto("yellow",0,$("#lad").val(),0),
				new ConfiguracionObjeto("green",0,$("#lvd").val(),0)
				];
			break;
		case "aforo":
	...
	contar();
	//actializa los objetos de tráffico
	var confs = new Array();
	...
	$.post("antragen.php",post,function(d){
		$("#prueba").html(d);
	},"text");
	eliminarchk();
	limpiartxt();
...
ObjetoTrafico.prototype.guardarDatos = function(){
	...
	var confs = new Array();
	var positions = this.getPosiciones();
	for(var x = 0;x<this.configuraciones.length;x++){
		confs[x] = this.configuraciones[x].getVars();
	}
	...
	$.post("antragen.php",enviarServidor,function(d){
		$("#prueba").html(d);
		$("#mensajes").html("Objeto Guardado");
		selectedFeature.id = d.ido;
