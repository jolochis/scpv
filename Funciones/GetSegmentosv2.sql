...
CREATE OR REPLACE FUNCTION "getSegmentos"(IN west numeric, IN north numeric, IN east numeric, IN south numeric, IN pjin numeric)
...
BEGIN
	...
	CREATE TEMPORARY TABLE auxIntersection (idCalle integer, segmentName text, 
		segmentType text, segmentClass text, coodinates text);
				...
				LOOP
					...
					ELSE
						...
						SELECT
							...
							END
						INTO direction
						FROM (Select distinct(osm_id),oneway FROM osm_planet_line where osm_id = idlinea) as p;
						...
						IF direction = -1 THEN
							UPDATE temp_intersections 
							SET "to"=cast('S'||auxidinter as text),SET "to2"=cast('S'||auxidinter as text) 
							WHERE name like intername AND idline2 = idlinea2 and idline1 = idlinea
							AND "type" like 'inter' ;
							
							UPDATE temp_intersections 
							SET "from"=cast('S'||auxidinter as text), "from2"=cast('S'||auxidinter as text)
							WHERE name like auxintername AND idline2 = auxidlinea2 AND idline1 = idlinea
							AND "type" like 'inter' ;
	...
	--4326
	return QUERY 
		SELECT "name",ST_AsText(St_transform(St_setSRID(interpunto::geometry,900913),$5)) as lonlat,interpunto,"to","from",0 as "length","type" FROM temp_intersections WHERE "type" like 'inter'
		union SELECT idsegmento as "name",
		ST_AsText(St_transform(St_setSRID(coordenadas::geometry,900913),$5)) as lonlat,coordenadas,"to","from",longitud as "length","type" from temp_segmentos
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION "getSegmentos"(numeric, numeric, numeric, numeric, numeric)
  OWNER TO postgres;