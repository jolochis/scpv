...
CREATE OR REPLACE FUNCTION "generateUtyilData"(IN west numeric, IN north numeric, IN east numeric, IN south numeric, IN pjin integer, IN propuesta integer)
...
BEGIN
	...
	LOOP	
		...
		ELSE
			...
			LOOP
				...
				ELSE
					...
					IF auxname IS NOT NULL THEN
						...
						VALUES(idlinea, auxname, 'intersection' ,cast('inter' as text),auxinter, $6,0.0);
					ELSE
						auxidinter = auxidinter + 1;
						--INSERT INTO temp_intersections(idline1,idline2,"type",coordinates,"name") 
						--VALUES(idlinea,auxidlinea,cast('inter' as text),auxinter,cast('I'||auxidinter as text));
						INSERT INTO segments("idCalle","segmentName","segmentType","type",coordinates,"idPropuesta",distance)
						VALUES(idlinea, cast('I'||auxidinter as text), 'intersection' ,cast('inter' as text),auxinter, $6,0.0);
					END IF;
			...
			IF auxname ISNULL THEN
				auxidinter = auxidinter + 1;
				INSERT INTO segments("idCalle","segmentName","segmentType","type",coordinates,"idPropuesta",distance)
				values(idlinea, cast('I'||auxidinter as text), 'intersection',cast('init' as text),
					ST_AsText(ST_StartPoint(linea::geometry)),$6, 0.0
				);
			END IF;
			...
			IF auxname ISNULL THEN
				auxidinter = auxidinter + 1;
				INSERT INTO segments("idCalle","segmentName","segmentType","type",coordinates,"idPropuesta",distance)
				values(idlinea, cast('I'||auxidinter as text), 'interseccion',cast('end' as text),
					ST_AsText(ST_EndPoint(linea::geometry)), $6, 0.0
				) ;
			END IF;
	...
	LOOP
		...
		ELSE
			...
			FROM segments 
			where "idCalle" = idlinea AND "segmentName" NOT LIKE '%S%'
				...
				LOOP
					...
					ELSE
						...
						INSERT INTO segments("idCalle","segmentName","segmentType","type",coordinates,"idPropuesta","distance")
							...
							$6,ST_Length(ST_Transform(ST_SetSRID(segment_linea::geometry,900913),26986)));
							
						INSERT INTO "segmentConnections"
							VALUES($6, cast('S'||auxidinter as text), intername);
						
						INSERT INTO "segmentConnections"
							VALUES($6, cast('S'||auxidinter as text), auxintername);
						...
						IF direction = 1 THEN
							INSERT INTO objetos (idprop, "type", positionscr, linescr, positiondst, linedst, segment)
								VALUES($6, 'direction', segment_linea, idlinea, segment_linea, idlinea, cast('S'||auxidinter as text))       
	...
	return QUERY 
		SELECT "segmentName" as "name",
			ST_AsText(St_transform(St_setSRID(coordinates::geometry,900913),$5)) as lonlat,
			coordinates, "distance" as "length", "segmentType","type" FROM segments 
			WHERE "type" like 'inter' AND "segmentName" like 'I%'
		union SELECT "segmentName" as "name",
			ST_AsText(St_transform(St_setSRID(coordinates::geometry,900913),$5)) as lonlat,
			coordinates,"distance" as "length", "segmentType","type" from segments
			WHERE "segmentName" like 'S%'
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION "generateUtyilData"(numeric, numeric, numeric, numeric, integer, integer)
  OWNER TO postgres;
