	...
	class Objects{
		...
		public function addDirection($id,$to){
			if(!key_exists("d{$id}",$this->objects)){
				$this->objects["d{$id}"]=array("type"=>"direction","id"=>"d{$id}","pos"=>"#","configs"=>array());
		...
		private function addDirConfiguration($id,$id2,$to){
			if(!key_exists("{$to}",$this->objects["{$id}"]["configs"]))
				$this->objects["{$id}"]["configs"]["{$to}"]=array(
				"id"=>"cfg{$id2}",
				"init"=>'0',
				"end"=>'$',
				"attr1"=>$to,
		...
		public function addConfiguration($oid,$id,$init,$end,$attr){
			if(!key_exists("cfg{$id}",$this->objects["{$oid}"]["configs"]))
				$this->objects["{$oid}"]["configs"]["cfg{$id}"]=array(
				"rid"=>"{$id}",
				"id"=>"cfg{$id}",
				"init"=>trim($init),
				"end"=>trim($end),
		...
		public function addState($oid,$id,$at1,$at2,$at3){			
			if(key_exists("cfg{$id}",$this->objects["{$oid}"]["configs"])){
				$t = sizeof($this->objects["{$oid}"]["configs"]["cfg{$id}"]["states"])+1;
				$this->objects["{$oid}"]["configs"]["cfg{$id}"]["states"]["attr{$t}"]=
					array(
						"attr1"=>trim($at1),
						"attr2"=>trim($at2),
						"attr3"=>trim($at3));
		...
		public function createUtyil(){
			if(sizeof($this->objects)>0)
			{
				...
				foreach($this->objects as $object => $value)
				{
					...
					$this->utyl.=">";
					if(sizeof($value["configs"]>0))
					{
						$this->utyl.="<configurations>";
						foreach($value["configs"] as $config=>$con){
